...
class tripletRankingLossLayer(Layer):
    ...
    def rankingLoss(self, input_example, input_positive, input_negative):
        ...
        loss = K.mean(K.maximum(0., self.confidence_margin -
...
class Repen_network:
    ...
    def __init__(self, hidden_dim=20, confidence_margin=1000.0):
        ...
        self.rankingloss = tripletRankingLossLayer(
    ...
    def compile_model(self, input_dim):
        ...
        hidden_layer = Dense(
            self.hidden_dim, activation='relu', name='hidden_layer')
        ...
        self.model = Model(
            inputs=[input_e, input_p, input_n], outputs=output_layer)
        self.model.compile(optimizer='adadelta',
                           loss=None)

    def get_representation(self):
        representation = Model(inputs=self.model.input[0],
                               outputs=self.model.get_layer('hidden_layer')
                               .get_output_at(0))
...
class Trainer:
    ...
    def __init__(self, n_epochs=50, batch_size=256,
                 ...
                 path_model=os.path.join(
                     os.getcwd(), 'autoad', 'algorithms', 'repen', 'model'),
                 save_suffix=None):
    ...
    def tripletBatchGeneration(self, X, rng,
        ...
        if (len(outlier_ids) == 2) and (type(outlier_ids) == list):
            ...
            neg_2 = rng.choice(outlier_ids[1], self.batch_size -
                               int(self.batch_size / 2),
                               p=negative_weights[1])
            negatives = np.hstack([neg_1, neg_2])
        else:
            negatives = rng.choice(
                outlier_ids, self.batch_size, p=negative_weights)
    ...
    def train(self, network, mode, x_train,
        ...
        model_name = os.path.join(
            self.path_model, 'REPEN_'+self.save_suffix+'.h5')

        checkpointer = ModelCheckpoint(model_name, monitor='loss', verbose=0,
                                       save_best_only=True,
                                       save_weights_only=True)

        network.model.fit_generator(self.batch_generator(
            x_train,
            positive_weights,
            negative_weights,
            inlier_indices,
...
class RepenModel:
    def __init__(self,
        ...
        self.Trainer = Trainer(n_epochs, batch_size,
                               nb_batch, random_seed, path_model, save_suffix)
    ...
    def prepare_data(self, x_train, y_train=None):
        ...
        if self.mode == "unsupervised":
            ...
            if outlier_ids.shape[0] < self.known_outliers:
                ...
                ind_scores = np.argsort(outlier_scores.flatten())
                ind_scores = [
                    elt for elt in ind_scores if elt not in outlier_ids]
            ...
            inlier_ids = np.delete(
                np.arange(len(x_train)), np.hstack(outlier_ids), axis=0)
            transforms = np.sum(
                outlier_scores[inlier_ids]) - outlier_scores[inlier_ids]
            total_weights_p = np.sum(transforms)
            ...
            if outlier_ids.shape[0] > self.known_outliers:
                ...
                remove_idx = self.Trainer.rng.choice(
                    outlier_ids, mn, replace=False)

                outlier_ids = np.array(
                    [elt for elt in outlier_ids if elt not in remove_idx])

            positive_weights = np.ones(
                inlier_ids.shape[0]) * (1 / inlier_ids.shape[0])
            negative_weights = np.ones(
                outlier_ids.shape[0]) * (1 / outlier_ids.shape[0])
    ...
    def fit(self, x_train, y_train=None, verbose=True):

        self.x_train = x_train
        self.prepare_data(x_train, y_train)
        self.network = self.Trainer.train(
            self.network,
            self.mode,
            x_train,
