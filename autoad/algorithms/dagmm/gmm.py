...
class GMM:
    ...
    def fit(self, z, gamma):
        ...
        with tf.compat.v1.variable_scope("GMM"):
            ...
            self.mu = mu = tf.einsum(
                'ik,il->kl', gamma, z) / gamma_sum[:, None]
    ...
    def energy(self, z):
        ...
        if self.training and self.phi is None:
            self.phi, self.mu, self.sigma, self.L = self.create_variable(
                z.shape[1])

        with tf.compat.v1.variable_scope("GMM_energy"):
            # Instead of inverse covariance
            # matrix, exploit cholesky decomposition
            # for stability of calculation.
            z_centered = z[:, None, :] - self.mu[None, :, :]  # ikl
            v = tf.compat.v1.matrix_triangular_solve(
                self.L, tf.transpose(z_centered, [1, 2, 0]))  # kli

            # log(det(Sigma)) = 2 * sum[log(diag(L))]
            log_det_sigma = 2.0 * \
                tf.reduce_sum(tf.compat.v1.log(
                    tf.compat.v1.matrix_diag_part(self.L)), axis=1)

            # To calculate energies, use "log-sum-exp"
            # (different from orginal paper)
    ...
    def cov_diag_loss(self):
        with tf.compat.v1.variable_scope("GMM_diag_loss"):
            diag_loss = tf.reduce_sum(
                tf.divide(1, tf.compat.v1.matrix_diag_part(self.sigma)))
