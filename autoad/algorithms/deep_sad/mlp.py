...
from autoad.algorithms.deep_sad.base_net import BaseNet
...
class MLP(BaseNet):
    ...
    def __init__(self, x_dim, h_dims=[128, 64], rep_dim=32, bias=False):
        ...
        layers = [Linear_BN_leakyReLU(
            neurons[i - 1], neurons[i], bias=bias)
...
class MLP_Autoencoder(BaseNet):
    def __init__(self, x_dim, h_dims=[128, 64], rep_dim=32, bias=False):
        ...
        self.decoder = MLP_Decoder(
            x_dim, list(reversed(h_dims)), rep_dim, bias)
...
class Linear_BN_leakyReLU(nn.Module):
    """
    A nn.Module that consists of a Linear layer followed
    by BatchNorm1d and a leaky ReLu activation
    """
